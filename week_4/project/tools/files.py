...
def write_file(path: str, content: str) -> str:
    ...
    try:
        ...
        console.print(f"\\n[bold red]WARNING:[/bold red] The agent wishes to WRITE to [bold cyan]{path}[/bold cyan]:")
        # preview content
        console.print(f"[dim]{content[:500]}[/dim]")
        if len(content) > 500:
            console.print("[dim]... (truncated)[/dim]")
            
        choice = input("Do you approve this write? [y/N]: ").strip().lower()
        if choice != 'y':
            return '{"error": "Human operator declined this write operation."}'
...
def edit_file(path: str, operation: str, start_line: int, end_line: int = None, content: str = "") -> str:
    ...
    try:
        ...
        preview_op = f"operation={operation}, start={start_line}, end={end_line}"
        console.print(f"\\n[bold red]WARNING:[/bold red] The agent wishes to EDIT [bold cyan]{path}[/bold cyan] ({preview_op}):")
        if content:
            console.print("[bold green]+ Content to insert:[/bold green]")
            console.print(f"[green]{content}[/green]")
            
        choice = input("Do you approve this edit? [y/N]: ").strip().lower()
        if choice != 'y':
            return '{"error": "Human operator declined this edit operation."}'
